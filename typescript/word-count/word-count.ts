...
export default class Words {
    public count(sentence: string): Map<string | number, number> {
        return this.getWords(sentence).reduce((counts: Map<string | number, number>, word: string) => {
    ...
    private getWords(sentence: string): string[] {
        return this.removeFormatting(sentence).split(" ").map((word) => word.trim()).filter((word) => word !== "")
    }
